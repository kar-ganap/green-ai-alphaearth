"""
Train Random Forest with Proper Cross-Validation

Uses 69D AlphaEarth features (3D annual magnitudes + 66D coarse landscape)
with stratified 5-fold CV for hyperparameter tuning to prevent overfitting.

Strategy:
1. Hyperparameter tuning via GridSearchCV with StratifiedKFold
2. Train final model on all 589 samples with best hyperparameters
3. Evaluate ONCE on 4 held-out validation sets (no peeking during tuning)

Usage:
    uv run python src/walk/11_train_random_forest.py
"""
...
from sklearn.metrics import roc_auc_score, accuracy_score, precision_score, recall_score, confusion_matrix, make_scorer
...
def main():
    print("=" * 80)
    print("RANDOM FOREST WITH STRATIFIED CROSS-VALIDATION")
    print("=" * 80)
    ...
    for set_name, data in val_data.items():
        ...
        baseline = baseline_logistic.get(set_name, 0.0)
        ...
        results[set_name] = {
            ...
            'improvement_vs_logistic': diff,
            'pct_change_vs_logistic': pct_change,
        ...
        if not np.isnan(roc_auc):
            print(f"  ROC-AUC:   {roc_auc:.3f}  (logistic: {baseline:.3f}, {diff:+.3f} / {pct_change:+.1f}%)")
        else:
            print(f"  ROC-AUC:   nan  (logistic: {baseline:.3f})")
    ...
    print(f"{'Validation Set':<20s} {'Logistic':>10s} {'RandomForest':>13s} {'Improvement':>12s}")
    print("-" * 80)
    for set_name in val_sets:
        if set_name in results:
            r = results[set_name]
            if not np.isnan(r['roc_auc']):
                print(f"{set_name:<20s} {r['baseline_logistic']:>10.3f} {r['roc_auc']:>13.3f} "
                      f"{r['improvement_vs_logistic']:>+12.3f} ({r['pct_change_vs_logistic']:>+5.1f}%)")
            else:
                print(f"{set_name:<20s} {r['baseline_logistic']:>10.3f}          nan              nan")
    ...
    print(f"Logistic Regression (69D): 0.583")

    if not np.isnan(edge_roc):
        print(f"Random Forest (69D): {edge_roc:.3f}")
        print(f"  5-Fold CV score: {grid_search.best_score_:.3f}")

        if edge_roc >= target:
            print(f"\n✓ TARGET MET: edge_cases ROC-AUC = {edge_roc:.3f} ≥ {target:.2f}")
            print(f"  Random Forest successfully improved performance!")
        else:
            gap = target - edge_roc
            print(f"\n✗ TARGET NOT MET: {gap:.3f} below target")
            improvement = edge_roc - 0.583
            if improvement > 0:
                print(f"  ✓ Random Forest improved by {improvement:+.3f} over logistic regression")
                print(f"  Next step: Try XGBoost")
            else:
                print(f"  ✗ Random Forest did not improve over logistic regression")
                print(f"  Next step: Add spatial features or try XGBoost")
    else:
        print(f"Random Forest (69D): nan")
    ...
    results_file = results_dir / 'random_forest_evaluation.json'
    with open(results_file, 'w') as f:
        json.dump(results, f, indent=2)
    print(f"✓ Results saved to: {results_file}")

    model_file = processed_dir / 'walk_model_random_forest.pkl'
