"""
Train Random Forest on Quick Win Augmented Dataset

Uses the augmented dataset (615 samples = 589 original + 26 new edge_cases samples)
...
Usage:
    uv run python src/walk/23_train_quickwin_rf.py
"""
...
def main():
    ...
    print("\nLoading augmented dataset features...")
    ...
    baseline_original = {
        ...
        'comprehensive': 0.711,
        'edge_cases': 0.583  # This is our target to improve
    }
    ...
    # Summary
    print(f"{'='*80}")
    print("QUICK WIN RESULTS SUMMARY")
    print(f"{'='*80}\n")

    print(f"{'Validation Set':<20s} {'Baseline':>10s} {'QuickWin':>10s} {'Improvement':>12s}")
    print("-" * 80)
    for set_name in val_sets:
        if set_name in results:
            r = results[set_name]
            if not np.isnan(r['roc_auc']):
                print(f"{set_name:<20s} {r['baseline_original']:>10.3f} {r['roc_auc']:>10.3f} "
                      f"{r['improvement_vs_baseline']:>+12.3f} ({r['pct_change_vs_baseline']:>+5.1f}%)")
            else:
                print(f"{set_name:<20s} {r['baseline_original']:>10.3f}        nan              nan")
    ...
    if not np.isnan(edge_roc):
        print(f"QuickWin (615 samples): {edge_roc:.3f}")
        print(f"  5-Fold CV score: {grid_search.best_score_:.3f}")

        improvement = edge_roc - baseline_edge

        if edge_roc >= target:
            print(f"\n✓ TARGET MET: edge_cases ROC-AUC = {edge_roc:.3f} ≥ {target:.2f}")
            print(f"  Quick win successful! Improved by {improvement:+.3f} ({improvement/baseline_edge*100:+.1f}%)")
            print(f"  Active learning approach validated")
        else:
            gap = target - edge_roc
            print(f"\n✗ TARGET NOT MET: {gap:.3f} below target")
            if improvement > 0:
                print(f"  ✓ Improved by {improvement:+.3f} ({improvement/baseline_edge*100:+.1f}%) over baseline")
                print(f"  Next step: Collect full 105 samples from shopping list")
            else:
                print(f"  ✗ No improvement over baseline")
                print(f"  Next step: Analyze collected samples, try different criteria")
    else:
        print(f"QuickWin (615 samples): nan")
    ...
    results_file = results_dir / 'quick_win_rf_evaluation.json'
    with open(results_file, 'w') as f:
        json.dump(results, f, indent=2)
    print(f"✓ Results saved to: {results_file}")

    model_file = processed_dir / 'walk_model_rf_quickwin.pkl'
