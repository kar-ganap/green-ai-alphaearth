"""
Train Random Forest on Complete Edge Cases Dataset

Uses the complete edge_cases dataset (636 samples = 589 original + 47 edge_cases)
to measure improvement on edge_cases validation set.

Baseline: 0.583 ROC-AUC (589 samples, 45.5% accuracy)
Quick Win: 0.600 ROC-AUC (615 samples, +2.9%)
Target: 0.65-0.70 ROC-AUC (636 samples)

Usage:
    uv run python src/walk/26_train_edge_cases_complete_rf.py
"""
...
def main():
    print("=" * 80)
    print("COMPLETE EDGE CASES: RANDOM FOREST WITH 636 SAMPLES")
    print("=" * 80)
    print("Baseline edge_cases: 0.583 ROC-AUC (589 samples, 45.5% accuracy)")
    print("Quick Win:           0.600 ROC-AUC (615 samples, +2.9%)")
    print("Target:              0.65-0.70 ROC-AUC (636 samples)")
    print()
    ...
    # Load edge_cases_complete features
    print("\nLoading complete edge_cases dataset features...")
    annual_path = processed_dir / 'walk_dataset_scaled_phase1_features_edge_cases_complete.pkl'
    ...
    print(f"\n✓ Complete edge_cases training set: {len(X_train)} samples, {X_train.shape[1]} features")
    ...
    for set_name, data in val_data.items():
        ...
        baseline = baseline_original.get(set_name, 0.0)
        quickwin = quickwin_scores.get(set_name, 0.0)
        diff_baseline = roc_auc - baseline if not np.isnan(roc_auc) else float('nan')
        diff_quickwin = roc_auc - quickwin if not np.isnan(roc_auc) else float('nan')
        pct_baseline = (diff_baseline / baseline * 100) if baseline > 0 and not np.isnan(diff_baseline) else float('nan')
        pct_quickwin = (diff_quickwin / quickwin * 100) if quickwin > 0 and not np.isnan(diff_quickwin) else float('nan')
        ...
        results[set_name] = {
            ...
            'quickwin_score': quickwin,
            'improvement_vs_baseline': diff_baseline,
            'improvement_vs_quickwin': diff_quickwin,
            'pct_change_vs_baseline': pct_baseline,
            'pct_change_vs_quickwin': pct_quickwin,
    ...
    # Summary
    print(f"{'='*80}")
    print("COMPLETE EDGE CASES RESULTS SUMMARY")
    print(f"{'='*80}\n")
    ...
    for set_name in val_sets:
        if set_name in results:
            ...
            if not np.isnan(r['roc_auc']):
                print(f"{set_name:<20s} {r['baseline_original']:>10.3f} {r['quickwin_score']:>10.3f} "
                      f"{r['roc_auc']:>10.3f} {r['improvement_vs_baseline']:>+10.3f} {r['improvement_vs_quickwin']:>+8.3f}")
            else:
                print(f"{set_name:<20s} {r['baseline_original']:>10.3f} {r['quickwin_score']:>10.3f}        nan           nan       nan")
    ...
    print(f"Baseline (589 samples):  {baseline_edge:.3f}")
    print(f"Quick Win (615 samples): {quickwin_edge:.3f}")

    if not np.isnan(edge_roc):
        print(f"Complete (636 samples):  {edge_roc:.3f}")
        print(f"  5-Fold CV score: {grid_search.best_score_:.3f}")

        improvement_baseline = edge_roc - baseline_edge
        improvement_quickwin = edge_roc - quickwin_edge

        if edge_roc >= target:
            print(f"\n✓ TARGET MET: edge_cases ROC-AUC = {edge_roc:.3f} ≥ {target:.2f}")
            print(f"  Improved by {improvement_baseline:+.3f} ({improvement_baseline/baseline_edge*100:+.1f}%) over baseline")
            print(f"  Improved by {improvement_quickwin:+.3f} ({improvement_quickwin/quickwin_edge*100:+.1f}%) over quick win")
            print(f"  Active learning approach validated!")
        else:
            gap = target - edge_roc
            print(f"\n✗ TARGET NOT MET: {gap:.3f} below target")
            if improvement_baseline > 0:
                print(f"  ✓ Improved by {improvement_baseline:+.3f} ({improvement_baseline/baseline_edge*100:+.1f}%) over baseline")
                if improvement_quickwin > 0:
                    print(f"  ✓ Improved by {improvement_quickwin:+.3f} ({improvement_quickwin/quickwin_edge*100:+.1f}%) over quick win")
                else:
                    print(f"  ✗ No improvement over quick win")
                print(f"\n  Next steps:")
                print(f"  - Current edge_cases samples: 47 (28 FN + 19 FP)")
                print(f"  - Consider collecting remaining rapid_response (25) and comprehensive (30) samples")
                print(f"  - May need more than just edge_cases samples to reach target")
            else:
                print(f"  ✗ No improvement over baseline")
                print(f"  - Possible overfitting to training set (CV: {grid_search.best_score_:.3f})")
    else:
        print(f"Complete (636 samples):  nan")
    ...
    results_file = results_dir / 'edge_cases_complete_rf_evaluation.json'
    with open(results_file, 'w') as f:
        json.dump(results, f, indent=2)
    print(f"✓ Results saved to: {results_file}")

    model_file = processed_dir / 'walk_model_rf_edge_cases_complete.pkl'
    with open(model_file, 'wb') as f:
        pickle.dump({
            ...
            'metadata': {
                ...
                'dataset': 'edge_cases_complete_636_samples'
            }
        }, f)
